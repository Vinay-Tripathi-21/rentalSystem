...
const ProfilePage = () => {
  ...
  const [userRentItem, setUserRentItem]= useState([]);
            ...
            userProduct?.map((product,idx)=>{
              return <>
               <div class="flex border-b border-gray-800 hover:bg-blue-500">
                  ...
                  <div class="p-3 w-full">{product.productName}</div>
                  <div class="p-3 w-full">{product.productPrice}</div>
                  <div class="p-3 w-full">{product.productImage.toString().substr(0,30) + '...'}</div>
...
<div class="p-3 w-12 flex-shrink-0 text-right"> {item.expireDate.substr(0,10) }</div>
</div></>;
})
